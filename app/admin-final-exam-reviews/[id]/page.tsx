...
export default function AdminFinalExamReviewDetailPage() {
  ...
  useEffect(() => {
    if (mounted && !authLoading && !user) {
      router.push(`/auth?redirect=/admin-final-exam-reviews/${encodeURIComponent(id || "")}`);
    }
  ...
  const saveAndFinalize = async () => {
    ...
    try {
      ...
      toast({ title: "Saved", description: "Final exam review finalized." });
      router.push("/admin-final-exam-reviews");
    } catch (e: any) {
  ...
  return (
    <AdminShell
      ...
      subtitle={lessonTitle ? `Lesson: ${lessonTitle}` : "Final exam submission"}
          ...
          {freeTextItems.length === 0 ? (
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-end gap-2">
                <Button variant="outline" onClick={() => router.push("/admin-final-exam-reviews")} disabled={saving}>
                  Back
                </Button>
