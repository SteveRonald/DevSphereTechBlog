...
export default function AdminFinalExamReviewsPage() {
  ...
  useEffect(() => {
    if (mounted && !authLoading && !user) {
      router.push("/auth?redirect=/admin-final-exam-reviews");
    }
  ...
  const openReview = (id: string) => {
    router.push(`/admin-final-exam-reviews/${id}`);
  };
  ...
  const loadSubmissions = async () => {
    try {
      ...
      const response = await fetch("/api/admin/final-exam-submissions?status=pending_review", {
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data?.error || "Failed to load final exam submissions");
      }
  ...
  return (
    <AdminShell
      ...
      subtitle="Review and grade final exam submissions"
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between gap-4">
              <CardTitle>Pending Final Exam Reviews</CardTitle>
                  ...
                  {submissions.length === 0 ? (
                    ...
                    submissions.map((s) => (
                      <TableRow key={s.id}>
                        ...
                        <TableCell className="max-w-[420px] truncate">{freeTextPreview(s.answers)}</TableCell>
