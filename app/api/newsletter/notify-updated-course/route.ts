...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: settings, error: settingsError } = await admin
      ...
      .select("course_update_notifications_enabled")
      .eq("id", 1)
      .single();
    ...
    if (!settings?.course_update_notifications_enabled) {
      return NextResponse.json({
        success: true,
        message: "Course update notifications are disabled",
        sent: 0,
        total: 0,
      });
    }
    ...
    const subject = `Course Updated: ${course.title} - CodeCraft Academy`;
            ...
            <tr>
              <td style="padding: 40px 20px; background-color: #ffffff; max-width: 600px; margin: 0 auto;">
                <div style="max-width: 560px; margin: 0 auto;">
                  <div style="background: linear-gradient(135deg, #0ea5e9 0%, #2563eb 100%); padding: 30px; border-radius: 12px; margin-bottom: 30px; text-align: center;">
                    <h2 style="color: #ffffff; margin: 0 0 10px 0; font-size: 26px;">Course updated</h2>
                    <p style="color: #ffffff; margin: 0; font-size: 16px; opacity: 0.95;">We made improvements to a course you might be following.</p>
                  </div>
                  ...
                  <div style="border: 1px solid #e5e7eb; border-radius: 12px; overflow: hidden; margin-bottom: 30px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                    ...
                    <div style="padding: 30px;">
                      ...
                      <p style="color: #4b5563; margin: 0 0 20px 0; font-size: 16px; line-height: 1.6;">
                        ${course.short_description || "The course has been updated with new content."}
                      </p>
                    </div>
                  </div>
                  ...
                  <div style="text-align: center; margin-bottom: 30px;">
                    <a href="${courseUrl}" 
                       style="background: #2563eb; color: #ffffff; padding: 16px 32px; text-decoration: none; border-radius: 8px; display: inline-block; font-weight: 600; font-size: 16px; box-shadow: 0 4px 6px rgba(37, 99, 235, 0.3);">
                      View Updates →
                    </a>
                  </div>

                  <div style="border-top: 1px solid #e5e7eb; padding-top: 30px; text-align: center;">
                    <p style="color: #6b7280; font-size: 14px; margin: 0 0 10px 0;">You're receiving this because you subscribed to course update emails.</p>
...
You're receiving this because you subscribed to course update emails.
Unsubscribe: ${siteUrl}/newsletter/unsubscribe?email={{EMAIL}}
    ...
    return NextResponse.json(
      {
        success: false,
        error: error.message || "Failed to send course update notifications",
      },
      { status: 500 }
    );
  }
