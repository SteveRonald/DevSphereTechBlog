...
export default function AdminProjectReviewsPage() {
  ...
  useEffect(() => {
    if (mounted && !authLoading && !user) {
      router.push("/auth?redirect=/admin-project-reviews");
    }
  ...
  const loadSubmissions = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(data?.error || "Failed to load project submissions");
      }
  ...
  const openReview = (id: string) => {
    router.push(`/admin-project-reviews/${id}`);
  };

  const urlPreview = (s: Submission) => {
    const u = (s.submission_url || "").trim();
    if (!u) return "—";
    return u;
  };
  ...
  return (
    <AdminShell
      ...
      subtitle="Review and approve project submissions"
